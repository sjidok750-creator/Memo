...
export async function browserCapture(req: CaptureRequest, emit: (e: CaptureEvent) => void, signal?: AbortSignal): Promise<{ memo: Memo; duplicate: boolean }> {
  ...
  if (req.replace) {
    const existing = demoStore.get(req.replace);
  ...
  if (req.kind === "youtube") {
    ...
    const dup = demoStore.list().find((m) => m.kind === "youtube" && m.source.videoId === videoId);
    ...
    emit({ type: "stage", id: "transcript", label: "브라우저에서는 자막을 못 읽어 웹에서 조사" });
    emit({ type: "stage", id: "summarize", label: "영상 내용 조사 후 요약 작성" });
    const { content, model } = await summarizeYouTube(info, null, signal);
  ...
  const { content, model } = await summarizePhoto({ mediaType: m[1], base64: m[2] }, note, signal);
  emit({ type: "stage", id: "save", label: "분야 분류 및 저장" });
...
async function resummarize(memo: Memo, emit: (e: CaptureEvent) => void, signal?: AbortSignal): Promise<Memo> {
  if (memo.kind === "youtube") {
    ...
    const { content, model } = await summarizeYouTube(info, null, signal);
    return demoStore.patch(memo.id, { ...content, model })!;
  }
  if (memo.kind === "book") {
    emit({ type: "stage", id: "summarize", label: "핵심 내용 요약 · 명문장 발췌" });
    const { content, model } = await summarizeBook(memo.source.query || memo.title, signal);
    return demoStore.patch(memo.id, { ...content, model })!;
  }
  ...
  emit({ type: "stage", id: "summarize", label: "사진 읽고 요약 작성" });
  const { content, model } = await summarizePhoto({ mediaType: m[1], base64: m[2] }, memo.source.note, signal);
  return demoStore.patch(memo.id, { ...content, model })!;
}

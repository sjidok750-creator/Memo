...
- **가로 넘침이 없어도 글자는 잘릴 수 있다.** 낱말 길이가 언어마다 달라(영어 "Summarize" 는 한국어 "요약" 의 두 배) 한국어로만 보면 못 찾는다. 세 언어 × 320·390·430px 에서 `scrollWidth > clientWidth` / `scrollHeight > clientHeight` 로 확인할 것 (밀어 보는 줄과 줄 수 제한을 건 곳은 뺀다). 빈 입력칸의 **안내 문구**는 이 방법으로 안 잡히므로 값에 넣어 보고 재야 한다 (`scratchpad/placeholder-fit.mjs`).
- 밀어 보는 줄(`overflow-x: auto`) 안은 검사에서 빼기 때문에 그 안에 든 것은 못 잡는다. 줄을 새로 만들 때 검색칸처럼 **줄어들 수 있는 입력칸을 같이 넣지 말 것** — 옆의 것이 커지면 입력칸만 찌그러진다 (필터가 다섯 개가 되자 영문 검색칸이 "S" 한 글자만 남았다). 모바일에서는 줄을 나눈다.

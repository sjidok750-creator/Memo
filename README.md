...
메모 상세 화면에서 제목·태그·분야를 고칠 수 있고, 요약이 마음에 들지 않으면 **다시 요약** 으로 같은 원본에서 새로 만들 수 있습니다. 홈 화면 아래의 **백업** 으로 모든 메모를 사진까지 포함한 JSON 하나로 내보내고, 다른 곳에서 다시 가져올 수 있습니다 (같은 메모는 건너뜁니다). 폰 브라우저의 "홈 화면에 추가"로 앱처럼 쓸 수 있습니다.
